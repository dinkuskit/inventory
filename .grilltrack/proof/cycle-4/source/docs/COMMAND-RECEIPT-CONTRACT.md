...
- returns current versions and an opaque, short-lived confirmation value bound
  to the normalized proposed action; and
- warns when the action is irreversible in place, while explaining that a
  correction creates another receipt.

Commit presents that confirmation value with the same proposed action. A stale
version or changed action is rejected and must be previewed again. Preview is a
safety and usability step, not a reservation and not a promise that later
commit must succeed.
