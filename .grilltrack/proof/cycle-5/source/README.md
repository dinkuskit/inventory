...
command with a real local SQLite test adapter. It proves atomic balance,
immutable-receipt, stable-result, replay, conflict, rejection, location, and
restart behavior. That adapter is explicitly development/test-only and refuses
production mode or in-memory use. It is not the final storage layer and makes
no production schema or deployment claim; the separately gated Cloudflare
SQLite Durable Object remains the production direction. There is still no
runtime service, installable plugin, executable CLI, or published npm package.
The package manifest remains private at `0.0.0` to prevent accidental
publication.
