...
digest of the opaque token, the normalized action digest, normalized principal,
issue/expiry timestamps, and an optional bound command ID. Plaintext tokens are
...
The disposable local SQLite adapter advances to
`opening-balance-local/v2` and adds
`inventory_opening_balance_confirmations`. Existing v1 test databases remain
