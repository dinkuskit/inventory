...
| `dinkus.stock-transfer` | Created transfer create/edit/cancel, dispatch, In-transit reopen, atomic whole receipt, contextual read, outgoing/expected/in-transit/on-hand effects, atomic replay/conflict, and immutable actor receipts | `src/features/stock-transfer/` | `src/features/stock-transfer/index.ts`; `src/index.ts` | `src/domain/exact-decimal.ts`; `src/domain/opening-balance.ts`; `src/storage/inventory-store.ts` | `tests/stock-transfer/`; `tests/cloudflare/stock-transfer.test.mjs`; `tests/cloudflare/inventory-pool.test.mjs` | `bin/verify-inventory quick` | `bin/verify-inventory full` | transfer commands, record, line and line-stock context, result, receipt, warning, read result, errors, normalization, digest, and execution/read factories | migrated feature |
