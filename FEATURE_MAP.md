...
| `dinkus.stock-reservation` | Named order-line holds, fail-closed available checks, one active hold per order/line, cancel-to-history release, reserved/available effects, atomic replay/conflict, and immutable actor receipts | `src/features/stock-reservation/` | `src/features/stock-reservation/index.ts`; `src/index.ts` | `src/domain/exact-decimal.ts`; `src/domain/opening-balance.ts`; `src/storage/inventory-store.ts` | `tests/stock-reservation/`; `tests/cloudflare/stock-reservation.test.mjs` | `bin/verify-inventory quick` | `bin/verify-inventory full` | reserve and release commands, reservation record, receipt, rejection codes, normalization, digest, and execution factories | migrated feature |
