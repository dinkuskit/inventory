...
function storedStockTransferListRowFrom(row: DatabaseRow) {
...
export class LocalSqliteTestInventoryStore implements InventoryStore {
	...
	async listStockTransfers(
			...
			selectedLocation = locationFrom(
				database.prepare(
					`SELECT pool_id, location_id, name, name_key, status, version,
					 ...
					 FROM inventory_locations
					 WHERE pool_id = ? AND location_id = ?`,
				).get(query.poolId, query.locationId) as DatabaseRow | undefined,
			);
		...
		bindings.splice(3, 0, query.poolId, query.poolId);
		return {
			selectedLocation,
			rows: (rows.all(...bindings) as DatabaseRow[]).map(
				storedStockTransferListRowFrom,
			),
		};
	}
