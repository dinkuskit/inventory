...
test("real stock-reservation proof stays local and reopens durable state", async () => {
	...
	assert.match(probe, /createPackStock/u);
	assert.match(probe, /V5_RESERVATION/u);
	...
	assert.match(assertion, /deepEqual\(replay\.result, commit\.pack\)/u);
	assert.match(assertion, /upgradedHold\.packedAt/u);
	assert.match(assertion, /\[5, 6\]/u);
